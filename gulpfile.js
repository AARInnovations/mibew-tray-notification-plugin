...
gulp.task('prepare-release', ['bower'], function() {
    ...
    return eventStream.merge(
        getSources()
            .pipe(zip('tray-notification-plugin-' + version + '.zip')),
        getSources()
            .pipe(tar('tray-notification-plugin-' + version + '.tar'))
            .pipe(gzip())
    )
    .pipe(chmod(0644))
...
var getSources = function() {
    ...
    .pipe(rename(function(path) {
        path.dirname = 'AARInnovations/Mibew/Plugin/TrayNotification/' + path.dirname;
    }));
}
